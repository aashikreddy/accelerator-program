...
MemoryUsage = df['MemoryUsagePct'].dtype

print(f"Pandas Vector Optimized Type: {MemoryUsage}")

lf = pl.from_pandas(df).lazy()
query = lf.filter(pl.col('MemoryUsagePct') > 75.0)
result = query.collect()

print("\nHigh Resource Violations Detected:")
for rows in result.iter_rows(named=False):
    print(*rows)
