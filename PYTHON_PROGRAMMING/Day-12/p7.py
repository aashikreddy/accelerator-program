...
import pandas as pd
import polars as pl
pandf=pd.read_csv("sales.csv")
pdresult=pandf.groupby("StoreCode")["Sales"].mean().astype(float)
print("Pandas Aggregated Performance Metrics")
print(pdresult.rename_axis(None).to_string())

poldf=pl.read_csv("sales.csv")
plresult=(
    poldf.lazy()
    .group_by("StoreCode")
