...
merge1 = pd.merge(customers, orders, on="CustomerID")
merge2 = pd.merge(merge1, restaurants, on="RestaurantID")

merge2["OrderDate"] = pd.to_datetime(merge2["OrderDate"])

merge2 = merge2.set_index("OrderDate")

print("Complete Order Report")

report = merge2.reset_index()
report = report.sort_values(by="OrderID")
...
monthly = merge2["OrderAmount"].resample("M").sum()


monthly.index = monthly.index.strftime("%Y-%m")

print("\nMonthly Revenue Report")
print(monthly.to_string(header=False))
